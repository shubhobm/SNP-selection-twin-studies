...
get.model = function(gene){
  ...
  for(isdn in 1:nsdn){
    ...
    for(j in 1:nt){
      ...
      best.index = c(which(colSums(out.mat) == nq), (p-4)+1:4) # add covariate columns to selected indices
      
      # get test error
      test.error = var(y[-train])
...
model.list = mclapply(1:9, loopfun, mc.cores=5)
## save separate files for each gene
for(i in 1:nrow(gene.info.df)){
  igene = paste(gene.info.df$genename[i])
  save(model.list[[i]], file=paste0("outMZDZ_",igene,".Rda"))
}
