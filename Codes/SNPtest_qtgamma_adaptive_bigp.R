...
get.outputs.depth = function(signal, rho,
  ...
  loopfun = function(rep){
    ...
    for(isdn in 1:nsdn){
      qt.list = step.nodepth.qtgamma(mod, sdn=sdn.vec[isdn], Emap=Emap,
                                     q.vec, t.vec, nboot=1e3)
      result.mat = matrix(0, nrow=5, ncol=nt)
      ...
      for(j in 1:nt){
        ...
        result.mat[2,j] = 96 - sum((1:p)[best.index] %in% (1:p)[-active0])
        result.mat[3,j] = sum(c(sum(1:6 %in% best.index)>0,
                             sum(7:10 %in% best.index)>0,
                             sum(91:96 %in% best.index)>0,
                             sum(97:100 %in% best.index)>0))
        result.mat[4,j] = 80 - sum((1:p)[best.index] %in% (1:p)[-active1])
    ...
    best.result = matrix(0, nrow=j, ncol=4)
    for(j in 1:nt){
      err.vec = lapply(result.list, function(x) x[5,j])
      best.sdn = which.min(err.vec)
      best.result[j,] = (result.list[[best.sdn]])[-5,j]/c(4,96,4,80)
    }
...
nrep = 1e3
t.vec = seq(.8, .5, by=-.03)
# get.outputs.depth(signal=.1, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h10_rho7_bigp.Rda")
get.outputs.depth(signal=.07, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h07_rho7_bigp.Rda")
get.outputs.depth(signal=.05, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h05_rho7_bigp.Rda")
get.outputs.depth(signal=.03, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h03_rho7_bigp.Rda")
get.outputs.depth(signal=.02, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h02_rho7_bigp.Rda")
get.outputs.depth(signal=.01, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h01_rho7_bigp.Rda")
get.outputs.depth(signal=0, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E2_h00_rho7_bigp.Rda")

Emap = "E1"
t.vec = exp(-(1:5))
get.outputs.depth(signal=.1, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h10_rho7_bigp.Rda")
get.outputs.depth(signal=.07, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h07_rho7_bigp.Rda")
get.outputs.depth(signal=.05, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h05_rho7_bigp.Rda")
get.outputs.depth(signal=.03, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h03_rho7_bigp.Rda")
get.outputs.depth(signal=.02, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h02_rho7_bigp.Rda")
get.outputs.depth(signal=.01, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h01_rho7_bigp.Rda")
get.outputs.depth(signal=0, rho=.7, Emap, sdn.vec, q.vec, t.vec, nrep, filename="E1_h00_rho7_bigp.Rda")