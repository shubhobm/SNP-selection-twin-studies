...
get.outputs.depth = function(signal, sdn.vec, rho, q, mult, nrep){
  ...
  loopfun = function(rep){
    ...
    nsdn = length(sdn.vec)
    TPTN.sdn = matrix(0, nrow=nsdn, ncol=9)
    ...
    for(i in 1:nsdn){
      ...
      bic.val = (mean(y^2))/mean(mod$r^2)
      
      if(length(best.index)>0){
        
        err = as.numeric(ytest - as.matrix(Xtest[,best.index]) %*% mod$beta.hat[best.index])
        test.var = sum(diag(cov(matrix(err, nrow=nfamily, byrow=F))))
        test.bias = sum(colMeans(matrix(err, nrow=nfamily, byrow=F))^2)
        
        # err = as.numeric(y - as.matrix(X[,best.index]) %*% mod$beta.hat[best.index])
        # bic.val = log(mean(err^2))/mean(mod$r^2) + log(n)/n*length(best.index)
        # gcv.val = mean(err^2)/(1-length(best.index)/n)^2
        
      }
      TPTN.sdn[i,] = c(TP.num, sum(TP.num), TN.num, length(best.index), test.bias, test.var)
    }
    
    best.sdn = which.min(TPTN.sdn[,9])
    TPTN.sdn[best.sdn,-9]
  }
