    ...
    for(isdn in 1:nsdn){
      ...
      for(i in 1:nq){
        for(j in 1:nt){
          ...
          TP1.vec = c(sum(1:6 %in% best.index))
...
q.vec = c(.9, .5, .2, .1, .05)
t.vec = c(.9, .8, .7, .6, .5)
get.outputs.depth(signal=.1, rho=.7, sdn.vec, q.vec, t.vec, nrep, filename="all_h10_rho07.Rda")
get.outputs.depth(signal=.05, rho=.7, sdn.vec, q.vec, t.vec, nrep, filename="all_h05_rho07.Rda")
get.outputs.depth(signal=.02, rho=.7, sdn.vec, q.vec, t.vec, nrep, filename="all_h02_rho07.Rda")
get.outputs.depth(signal=.01, rho=.7, sdn.vec, q.vec, t.vec, nrep, filename="all_h01_rho07.Rda")
get.outputs.depth(signal=0, rho=.7, sdn.vec, q.vec, t.vec, nrep, filename="all_h00_rho07.Rda")
