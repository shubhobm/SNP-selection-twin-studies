...
nfamily = 100 # number of qqfamilies
...
get.outputs.depth = function(signal, sdn.vec, rho, q, mult, nrep){
  ...
  loopfun = function(rep){
    ...
    for(i in 1:nsdn){
      ...
      TN.num = 30 - sum((1:p)[best.index] %in% (1:p)[-active.ind])
  ...
  z = c(apply(TPTN.mat,2,mean)/c(rep(1,4),4,30,1),
        apply(TPTN.mat,2,sd)/c(rep(1,4),4,30,1))
...
get.outputs.others = function(signal, rho, adj, nrep){
  ...
  loopfun = function(rep){
    ...
    TN.BIC = 30- sum((1:p)[best.BIC] %in% (1:p)[-active.ind])
    ...
    TP.num.RF = c((sum(best.RF %in% 1:6)>0), (sum(best.RF %in% 7:10)>0),
                           (sum(best.RF %in% 41:46)>0), (sum(best.RF %in% 47:50)>0))
    TN.num.RF = 30 - sum((1:p)[best.RF] %in% (1:p)[-active.ind])
  ...
  z = c(apply(TPTN.mat.RF,2,mean)/c(rep(1,4),4,30,1),
        apply(TPTN.mat.RF,2,sd)/c(rep(1,4),4,30,1),
        apply(TPTN.mat.BIC,2,mean)/c(rep(1,4),4,30,1),
        apply(TPTN.mat.BIC,2,sd)/c(rep(1,4),4,30,1))
...
adj = 'fdr'
active.ind = active.ind = c(1:10,41:50)
p.active = 4
outfun(rho=.7, adj=adj, nrep, q=.9, mult=.6, filename='all_q9_mult6r.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.5, mult=.6, filename='all_q5_mult6.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.2, mult=.6, filename='all_q2_mult6.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.1, mult=.6, filename='all_q1_mult6.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.05, mult=.6, filename='all_q05_mult6.Rda')
# 
# outfun(rho=.7, adj=adj, nrep, q=.9, mult=.5, filename='all_q9_mult5.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.5, mult=.5, filename='all_q5_mult5.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.2, mult=.5, filename='all_q2_mult5.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.1, mult=.5, filename='all_q1_mult5.Rda')
# outfun(rho=.7, adj=adj, nrep, q=.05, mult=.5, filename='all_q05_mult5.Rda')