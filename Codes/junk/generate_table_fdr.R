...
for(i in 1:5){
  ...
  load(paste0("others_h",h,"_rho07_big.Rda"))
  mean.mat = round(apply(simplify2array(all.list), 1:2, mean),2)
  
  TP0.df[index.i[1],1:2] = mean.mat[1,]
  TN0.df[index.i[1],1:2] = mean.mat[2,]
  TP1.df[index.i[1],1:2] = mean.mat[3,]
  TN1.df[index.i[1],1:2] = mean.mat[4,]
  
  # add evalue values
  load(paste0("fdr_h",h,"_rho7.Rda"))
  mean.mat = round(apply(simplify2array(all.list), 1:2, mean),2)
